...
import { updateSkillAccess } from '@/lib/api/endpoints/skills'
...
export function AccessControlToggle({
  ...
  const [isPublic, setIsPublic] = useState(skill.access === 'PUBLIC')
  ...
  const handleToggle = async (checked: boolean) => {
    ...
    try {
      await updateSkillAccess(orgName, skill.name, newAccess)
      setIsPublic(checked)
      onAccessChange?.(newAccess)
      toast.success(
        `Skill is now ${checked ? 'public' : 'private'}`,
        {
          description: checked
            ? 'Anyone can view and install this skill'
            : 'Only organization members can view and install this skill',
        }
      )
    } catch (error) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Control who can view and install this skill
        </CardDescription>
      </CardHeader>
