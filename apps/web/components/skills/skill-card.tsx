...
export function SkillCard({ skill }: SkillCardProps) {
  ...
  const isPrivate = skill.access === 'PRIVATE'

  return (
    <Link href={`/skill/${orgName}/${skill.name}`}>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-start gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <CardTitle className="flex items-center gap-2 text-base md:text-lg truncate text-white" style={{ fontFamily: 'var(--font-della-respira)' }}>
                <span className="truncate">@{orgName}/{skill.name}</span>
          ...
          <div className="flex items-center gap-4 text-xs text-gray-400 font-body">
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-medium text-white">{(skill.downloads || 0).toLocaleString()}</span>
            </div>
            <div className="flex items-center gap-1">
              <span>Updated {skill.updatedAt ? formatDistanceToNow(new Date(skill.updatedAt), { addSuffix: true }) : 'recently'}</span>
            </div>
          </div>
        </CardHeader>
